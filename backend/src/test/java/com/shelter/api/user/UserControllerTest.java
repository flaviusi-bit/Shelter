...
class UserControllerTest {
    ...
    void updateUserDisplayNameAndRole() {
        ...
        UserController.UserView result = controller.update(user.getId(), new UserController.UpdateUserRequest("Shelter Coordinator", "COORDINATOR"), auth);

        assertEquals("Shelter Coordinator", result.displayName());
        assertEquals("COORDINATOR", result.role());
        verify(audit).record("admin", "UPDATE_USER", "USER", user.getId(), "username=volunteer, role=COORDINATOR");
    }
