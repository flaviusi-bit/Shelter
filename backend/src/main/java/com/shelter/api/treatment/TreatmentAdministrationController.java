...
public class TreatmentAdministrationController {
    ...
    public List<TreatmentAdministration> list(@PathVariable UUID animalId,@PathVariable UUID treatmentId){
        ensureTreatment(animalId,treatmentId); return administrations.findByTreatmentIdOrderByScheduledAtAsc(treatmentId);
    }

    @PostMapping("/generate")
    @ResponseStatus(HttpStatus.CREATED)
    public List<TreatmentAdministration> generate(@PathVariable UUID animalId,@PathVariable UUID treatmentId,@RequestParam(defaultValue="14") int days){
        ...
        while(cursor.isBefore(limit)&&created.size()<1000){
            if(!existingTimes.contains(cursor)){var a=new TreatmentAdministration();a.setTreatment(treatment);a.setScheduledAt(cursor);created.add(administrations.save(a));}
    ...
    public TreatmentAdministration administer(@PathVariable UUID animalId,@PathVariable UUID treatmentId,@PathVariable UUID administrationId,@RequestBody(required=false) ActionRequest request,Authentication authentication){
        ensureTreatment(animalId,treatmentId);
        TreatmentAdministration a=administrations.findById(administrationId).orElseThrow(()->new IllegalArgumentException("Administration not found: "+administrationId));
        if(!a.getTreatment().getId().equals(treatmentId))throw new IllegalArgumentException("Administration does not belong to treatment");
        a.setStatus("ADMINISTERED");a.setAdministeredAt(OffsetDateTime.now());a.setAdministeredBy(authentication.getName());
        if(request!=null)a.setNotes(request.notes());
        return administrations.save(a);
    }

    @PostMapping("/{administrationId}/status")
    public TreatmentAdministration setStatus(@PathVariable UUID animalId,@PathVariable UUID treatmentId,@PathVariable UUID administrationId,@RequestBody ActionRequest request){
        ensureTreatment(animalId,treatmentId);
        TreatmentAdministration a=administrations.findById(administrationId).orElseThrow(()->new IllegalArgumentException("Administration not found: "+administrationId));
        if(!a.getTreatment().getId().equals(treatmentId))throw new IllegalArgumentException("Administration does not belong to treatment");
        if(!List.of("MISSED","SKIPPED").contains(request.status()))throw new IllegalArgumentException("Invalid status");
        a.setStatus(request.status());a.setNotes(request.notes());return administrations.save(a);
    }
    ...
    private Treatment ensureTreatment(UUID animalId,UUID treatmentId){
        ...
        return treatments.findById(treatmentId).filter(t->t.getAnimal().getId().equals(animalId)).orElseThrow(()->new IllegalArgumentException("Treatment not found: "+treatmentId));
    }
    ...
    private Duration parseFrequency(String frequency){
        ...
        var m=java.util.regex.Pattern.compile("every\\s+(\\d+)\\s+hours?").matcher(f);if(m.matches())return Duration.ofHours(Long.parseLong(m.group(1)));
        var d=java.util.regex.Pattern.compile("every\\s+(\\d+)\\s+days?").matcher(f);if(d.matches())return Duration.ofDays(Long.parseLong(d.group(1)));
